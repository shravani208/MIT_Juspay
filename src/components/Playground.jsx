...
const blockData = [
  { type: "move", category: "motion", initialValue: { x: 20, y: 0 } },
  {
    type: "clockwise",
    category: "motion",
    initialValue: { x: 0, y: 0, rotation: 30 },
  },
  {
    type: "anticlockwise",
    category: "motion",
    initialValue: { x: 10, y: 10, rotation: 30 },
  },
  {
    type: "go_to",
    category: "motion",
    initialValue: { delay: 2, x: 30, y: 30, rotation: 0, message: "", size: 0 },
  },
  {
    type: "glide",
    category: "motion",
    initialValue: { size: 0, delay: 2, x: 25, y: 25, rotation: 0 },
    onAction: "consoleLog",
  },
  ...
  { type: "mouse_pointer", category: "motion", initialValue: {} },
  {
    ...
    category: "looks",
    initialValue: { size: 0, delay: 5, message: "Hello" },
  },
  {
    type: "say",
    category: "looks",
    initialValue: { delay: 1, rotation: 0, size: 0, message: "Hello" },
  },
  {
    type: "change_size",
    category: "looks",
    initialValue: { x: 0, y: 0, delay: 0, rotation: 0, size: 5, message: "" },
  },
...
function Playground() {
  ...
  const onDragEnd = (result) => {
    ...
    if (source.droppableId === destination.droppableId) {
      if (destination.index === 0) {
        const exist = newGroup[destinationGroupIndex].items.find(
          (each) =>
            each.type === "flag_clicked" ||
            each.type === "space_clicked" ||
            each.type === "sprite_clicked"
        );
        if (exist) {
          return;
        }
      }
      const [movedItem] = newGroup[sourceGroupIndex].items.splice(
        source.index,
        1
      );
      if (
        movedItem?.type === "flag_clicked" ||
        movedItem?.type === "space_clicked" ||
        movedItem?.type === "sprite_clicked"
      ) {
        newGroup[destinationGroupIndex].items.splice(0, 0, movedItem);
        return;
      }

      newGroup[destinationGroupIndex].items.splice(
        destination.index,
        0,
        movedItem
      );
      ...
      if (!isNaN(sourceGroupIndex) && !isNaN(destinationGroupIndex)) {
        ...
        if (
          blockDetails?.type === "flag_clicked" ||
          blockDetails?.type === "space_clicked" ||
          blockDetails?.type === "sprite_clicked"
        ) {
          const exist = newGroup[destinationGroupIndex].items.find(
            (each) =>
              each.type === "flag_clicked" ||
  ...
  const handleClickAction = (event) => {
    ...
    setData({
      ...data,
      clicked: { ...getClicked, click_id: Date.now().toString(16) },
    });
  };
  ...
  return (
    <div className="p-4 w-full">
      <div className="flex flex-row justify-between mr-4">
        ...
        <div
          className="self-center cursor-pointer border-1 rounded-md p-1 text-black flex flex-row gap-2"
          ...
          <Droppable droppableId="draggableItems" isDropDisabled={true}>
            {(provided) => (
              <div
                ...
                className="draggable_container h-[calc(100vh_-_6rem)] overflow-y-scroll w-full"
              >
              ...
              >
                {(provided) => (
                  <div
                    ref={provided.innerRef}
                    {...provided.droppableProps}
                    className="bg-gray-100 w-full flex flex-col  p-4 rounded-md droppable_area"
                  >
                    {groupIndex > 0 && (
                      <div
                    ...
                    {group.items.length === 0 && (
                      <p className="font-semibold py-44 text-lg text-gray-400 text-center">
                        Add/Drop actions Here
                      </p>
                    )}
                    {group.items.length > 0 && (
                      <p className="text-xs text-right text-gray-400 pv-2">
                        *you can delete the block by dragging outside
                      </p>
                    )}
